...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    extern __shared__ __half tileMem[]; // shared memory

    #define out_4d(i3, i2, i1, i0) output[(i3) * (Map_out * Height_out * Width_out) + (i2) * (Height_out * Width_out) + (i1) * (Width_out) + i0]
    #define in_4d(i3, i2, i1, i0) input[(i3) * (Channel * Height * Width) + (i2) * (Height * Width) + (i1) * (Width) + i0]
    #define mask_4d(i3, i2, i1, i0) kernel_mask[(i3) * (Channel * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    #define sharedMem(i2, i1, i0) tileMem[(i2)*(blocksize*blocksize) + (i1) * blocksize + (i0)] // use for 3d shared memory
        ...
        for(int i = ty; i < blocksize; i+= TILE_WIDTH){
            for(int j = tx; j < blocksize; j += TILE_WIDTH){
                if(h_topleft + i < Height && w_topleft + j < Width){
                    sharedMem(c,i,j) = __float2half_rn(in_4d(n, c, h_topleft + i, w_topleft + j));
                }
                else{
                    sharedMem(c,i,j) = __float2half_rn(0.0f);
                }
            }
        }
    }
    ...
    if (h < Height_out && w < Width_out){   
        for(c = 0; c < Channel; ++c){  
            for(int p = 0; p < K; ++p){     // for loop, the mask K x K
                for(int q = 0; q < K; ++q){
                    acc = __hadd(acc, __hmul(sharedMem(c, ty + p, tx + q), __float2half_rn(mask_4d(m,c,p,q))));                    // acc += in_4d(n, c, h+p, w+q) * mask_4d(m,c,p,q);
                }
            }
        }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    conv_forward_kernel<<<DimGrids, DimBlocks, Channel * blocksize * blocksize *sizeof(__half)>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
