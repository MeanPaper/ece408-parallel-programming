...
#define MASK_SIZE   7 
__constant__ float kernel_mask[3136];
    ...
    if(ty < TILE_WIDTH && tx < TILE_WIDTH){        //     for(int p = 0; p < K; ++p){     // for loop, the mask K x K
        ...
        for(int c = 0; c < Channel; ++c){
            for(int p = 0; p < K; ++p){     // for loop, the mask K x K
                for(int q = 0; q < K; ++q){
                    acc += sharedMem(c, ty + p, tx + q) * mask_4d(m,c,p,q);
                }
            }
        }
    }
...
{    
    ...
    int Z = H_grid * W_grid;     
    int blocksize = TILE_WIDTH + K - 1;               // h x w requirements for the image
    dim3 DimBlocks(blocksize, blocksize, 1); 
