...
#define max_masks   16
#define min_masks   4
#define CACHE_SIZE  (TILE_WIDTH + MASK_SIZE - 1)
...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    const int shared_width = CACHE_SIZE;
    ...
    #define sharedMem(i2, i1, i0) tileMem[(i2) * (shared_width*shared_width) + (i1)*(shared_width) + i0]
    ...
    for(int c = 0; c < Channel; ++c){
        if( h < Height && w < Width){
            sharedMem(c, ty, tx) = in_4d(n, c, h, w);
        }
        else{
            sharedMem(c, ty, tx) = 0.0f;
        }
    }
    __syncthreads();
    
    if(ty < TILE_WIDTH && tx < TILE_WIDTH){        //     for(int p = 0; p < K; ++p){     // for loop, the mask K x K

        for(int c = 0; c < Channel; ++c){
            for(int p = 0; p < K; ++p){     // for loop, the mask K x K
                for(int q = 0; q < K; ++q){
                    __half temp = __hmul(__float2half_rn(sharedMem(c, ty + p, tx + q)), __float2half_rn(mask_4d(m,c,p,q)));
                    acc = __hadd(acc, temp);
                }
            }
        }
...
{    
    ...
    int H_grid = ceil(((float)(Height - K + 1))/TILE_WIDTH);  // height grids 
    int W_grid = ceil(((float)(Width - K + 1))/TILE_WIDTH);  // width grids
    int Z = H_grid * W_grid;                    // h x w requirements for the image
    dim3 DimBlocks(CACHE_SIZE, CACHE_SIZE, 1); 
    dim3 DimGrids(Map_out, Z, Batch);         // grid dimension

    conv_forward_kernel<<<DimGrids, DimBlocks, Channel * CACHE_SIZE * CACHE_SIZE * sizeof(float)>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
